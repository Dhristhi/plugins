...
const StyledChip = styled(Chip)(({ theme, status }) => {
  ...
  if (status) {
    const color = statusColors[status.toLowerCase()] ?? theme.palette.primary.light;
    return {
      padding: 0,
      color: color,
      borderRadius: 4,
...
const CustomLabelControl = (props) => {
  ...
  const renderData = () => {
    ...
    if (Array.isArray(data) && data.length > 0) {
      return data.map((sk, j) => <StyledChip key={j} label={sk} size="small" sx={{ m: 0.2 }} />);
    }
  ...
  return (
    <Grid item>
      <Box p={0}>
        <Typography variant="body2" gutterBottom sx={{ color: theme.palette.drawerLabel }}>
          {label}
        </Typography>
