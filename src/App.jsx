...
const App = () => {
  ...
  const buildSchemaFromFields = useCallback((fieldsArray, parentKey = null) => {
    const properties = {};
    const required = [];
    const nestedObjects = {};
  ...
  const buildUISchemaForArrayItems = useCallback((fieldsArray) => {
    return fieldsArray
  ...
  const buildUISchemaFromFields = useCallback(
    (fieldsArray, parentKey = null) => {
      return fieldsArray
        .filter((field) => !field.uischema?.options?.hidden)
        .map((field) => {
          if (field.isLayout) {
            const uischema = {};
            if (field.icon) uischema.icon = `Icon${field.icon}`;
            uischema.type =
    ...
    [buildUISchemaForArrayItems]
  );

  const createDefaultArrayItem = useCallback((children) => {
    const item = {};
  ...
  const initializeNestedFormData = useCallback(
    (fieldsArray, parentData = {}) => {
      const data = { ...parentData };

      fieldsArray.forEach((field) => {
        if (!field.isLayout) {
          if (!(field.key in data)) {
            if (field.schema.type === "boolean") data[field.key] = false;
            else if (field.schema.type === "number") data[field.key] = 0;
          ...
          if (!(field.key in data)) data[field.key] = {};
          if (field.children) {
  ...
  }, [fields, buildUISchemaFromFields]);
  ...
  const updateFieldById = (fieldsArray, updatedField) => {
    return fieldsArray.map((field) => {
      ...
      if (field.children && field.children.length > 0) {
        ...
        return newChildren !== field.children
  ...
  const handleReorderFields = (activeIdArg, overId, overData) => {
    setFields((prevFields) => {
      const newFields = [...prevFields];
      const activeField = findFieldById(newFields, activeIdArg);
      const overField = findFieldById(newFields, overId);
      if (!activeField || !overField) return prevFields;
      removeFieldById(newFields, activeIdArg);
  ...
  const handleDragStart = (event) => {
    ...
    if (active.data.current?.type === "palette-item") {
      ...
      setDraggedItem({ type: "palette-item", fieldType });
    } else {
  ...
  const customCollisionDetection = (args) => {
    ...
    if (dropZones.length > 0) {
      const dropZoneCollisions = rectIntersection({
        ...args,
        droppableContainers: new Map(dropZones.map((z) => [z.id, z])),
      });
  ...
  const getAllFieldIds = (fieldsArray) => {
    ...
    fieldsArray.forEach((field) => {
      ids.push(field.id);
      if (field.children) ids.push(...getAllFieldIds(field.children));
    });
    return ids;
  };
  ...
  const convertSchemaToFields = useCallback((schema) => {
    ...
    Object.entries(schema.properties).forEach(([key, property]) => {
      ...
      if (property.enum) newField.schema.enum = property.enum;
                ...
                {!showSchemaEditor && (
                  ...
                  >
                    {showFormPreview ? (
                      ...
                      <FormPreview
                        formState={formState}
                        onDataChange={(newData) => setFormData(newData)}
                        showFormPreview={showFormPreview}
                        setShowFormPreview={setShowFormPreview}
                        showSchemaEditor={showSchemaEditor}
                        setShowSchemaEditor={setShowSchemaEditor}
